...
| OpenCode | TypeScript plugin, OpenCode v2 API (`ctx.shell.hook("create.before")`) | Yes |
...
Creates `~/.config/opencode/plugins/rtk.ts` (OpenCode v2 plugin API). Uses the `ctx.shell.hook("create.before")` hook. Alternatively, add `github:iqhive/rtk#opencode-v2.0.2` to `plugins` in `opencode.jsonc`.
